...
@router.get(
    "/groups/{group_id}/applicants", response_model=ApplicantsListResponse
)
def list_applicants(
    group_id: uuid.UUID,
    ...
    rows = db.execute(
        select(GroupApplicants)
        .where(GroupApplicants.group_id == group_id)
        .order_by(GroupApplicants.created_at.desc())
    ).scalars()
    return ApplicantsListResponse(
        applicants=[applicant_to_response(r) for r in rows]
    )


@router.post(
    "/groups/{group_id}/applicants", response_model=ApplicantResponse
)
def create_applicant(
    ...
    applicant = GroupApplicants(
        ...
        role_context=(
            payload.role_context.strip() if payload.role_context else None
        ),
        notes=payload.notes,
...
def update_applicant(
    ...
    if "name" in data:
        applicant.name = (
            data["name"].strip() if isinstance(data["name"], str) else data["name"]
        )
    if "email" in data:
        applicant.email = str(data["email"]) if data["email"] else None
    if "phone" in data:
        applicant.phone = (
            data["phone"].strip() if isinstance(data["phone"], str) else data["phone"]
        )
    if "status" in data and data["status"] is not None:
        applicant.status = data["status"]
    if "role_context" in data:
        applicant.role_context = (
            data["role_context"].strip()
            if isinstance(data["role_context"], str)
            else data["role_context"]
        )
    if "notes" in data:
...
@router.delete(
    "/groups/{group_id}/applicants/{applicant_id}", status_code=204
)
...
@router.get(
    "/public/applicants/{token}", response_model=PublicApplicantPreviewResponse
)
def preview_public_applicant(token: str, db: Session = Depends(get_db)):
    applicant = db.execute(
        select(GroupApplicants).where(GroupApplicants.self_reg_token == token)
    ).scalar_one_or_none()
    if applicant is None:
    ...
    expired = (
        applicant.self_reg_token_expires_at is None
        or applicant.self_reg_token_expires_at <= utc_now()
    )
...
@router.post(
    "/public/applicants/{token}", response_model=PublicApplicantSubmitResponse
)
