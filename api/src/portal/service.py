...
def _roommate_snapshot_dict(rp: Any) -> dict[str, Any]:
    return {
        ...
        "smoking": rp.smoking,
        "languages_spoken": list(rp.languages_spoken or []),
...
def get_roommate_state(db: Session, user_id: uuid.UUID) -> dict[str, Any]:
    ...
    my_profile = {
        ...
        "languages_spoken": list(prof.languages_spoken or []) if prof else [],
        "preferred_languages": list(prof.preferred_languages or []) if prof else [],
        "must_have_preferred_languages": prof.must_have_preferred_languages if prof else False,
    ...
    for c in conns:
        ...
        roommate = {
            ...
            "languages_spoken": list(snap.get("languages_spoken") or []),
...
def replace_roommates(db: Session, user_id: uuid.UUID, payload: RoommateStatePayload) -> None:
    ...
    db.add(
        RoommateProfiles(
            ...
            languages_spoken=list(mp.languages_spoken or []),
            preferred_languages=list(mp.preferred_languages or []),
            must_have_preferred_languages=bool(mp.must_have_preferred_languages),
