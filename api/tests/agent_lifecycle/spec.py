...
META = [
    PromptCase(
        stage="meta", id="meta_what_can_you_help",
    ...
    PromptCase(
        stage="meta", id="meta_get_started",
        prompt="how do I get started?",
        notes="Onboarding-shaped meta. Navigator onboarding is acceptable; "
              "orchestrator-only is also fine.",
        forbidden_agents={"search_agent"},
    ),
    PromptCase(
        stage="meta", id="meta_actually_book",
...
ONBOARDING = [
    PromptCase(
        stage="onboarding", id="ob_brand_new",
...
SEARCH = [
    PromptCase(
        stage="discovery", id="search_2br_williamsburg",
    ...
    PromptCase(
        stage="discovery", id="search_show_dishwasher",
    ...
    PromptCase(
        stage="discovery", id="search_under_3k_wd",
        prompt="anything under 3k with a washer/dryer?",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_doorman_central_park",
        prompt="I want a doorman building near Central Park",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_quiet_top_floor",
        prompt="quiet block, 1BR, top-floor, under 3800",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_pet_studio_lic",
        prompt="pet-friendly studios in LIC",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_compare_three",
    ...
    PromptCase(
        stage="discovery", id="search_near_l_train",
        prompt="what's available near the L train?",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_parking_included",
        prompt="I need parking included",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_outdoor_space",
        prompt="something with outdoor space under 5k",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_posted_this_week",
        prompt="only show me listings posted this week",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_drop_budget",
    ...
    PromptCase(
        stage="discovery", id="search_near_school",
        prompt="show me places close to a good elementary school",
        expected_agents={"search_agent"},
        expected_tools={"search_listings"},
    ),
    PromptCase(
        stage="discovery", id="search_no_ground_floor",
    ...
    PromptCase(
        stage="discovery", id="search_include_jc",
...
SAVING = [
    PromptCase(
        ...
        notes="After a save, the turn STOPS. No chained search, no extra "
              "property_results cards.",
    ),
    PromptCase(
        stage="saving", id="just_save_no_chain",
        prompt="just save",
        context_from=["search_2br_williamsburg"],
    ...
    PromptCase(
        stage="saving", id="save_second_also",
    ...
    PromptCase(
        stage="saving", id="saved_remove_bushwick",
    ...
    PromptCase(
        stage="saving", id="saved_cheapest",
        prompt="which of my saved places is cheapest?",
        expected_agents={"saved_agent"},
        expected_tools={"list_saved_properties"},
    ),
    PromptCase(
        stage="saving", id="saved_with_laundry",
        prompt="which saved places have laundry in-unit?",
        expected_agents={"saved_agent"},
        expected_tools={"list_saved_properties"},
    ),
    PromptCase(
        stage="saving", id="saved_clear_all",
...
TOURS = [
    PromptCase(
        stage="touring", id="tour_schedule_first_tomorrow",
    ...
    PromptCase(
        stage="touring", id="tour_book_second_saturday",
    ...
    PromptCase(
        stage="touring", id="tour_upcoming",
    ...
    PromptCase(
        stage="touring", id="tour_reschedule_thursday",
        prompt="reschedule my Thursday tour to Friday",
        expected_agents={"tours_agent"},
    ),
    PromptCase(
        stage="touring", id="tour_cancel_4pm",
    ...
    PromptCase(
        stage="touring", id="tour_completed_list",
        prompt="which tours have I already completed?",
        expected_agents={"tours_agent"},
        expected_tools={"list_my_tours"},
    ),
    PromptCase(
        stage="touring", id="tour_add_note",
    ...
    PromptCase(
        stage="touring", id="tour_this_week",
        prompt="show me tours this week",
        expected_agents={"tours_agent"},
        expected_tools={"list_my_tours"},
    ),
    PromptCase(
        stage="touring", id="tour_mark_completed",
    ...
    PromptCase(
        stage="touring", id="tour_next_address",
...
PROFILE = [
    PromptCase(
        stage="profile", id="profile_budget",
    ...
    PromptCase(
        stage="profile", id="profile_set_rent_5000",
        prompt="set my max rent to 5000",
        expected_agents={"profile_agent"},
        expected_tools={"update_my_profile"},
    ),
    PromptCase(
        stage="profile", id="profile_live_in_jc",
    ...
    PromptCase(
        stage="profile", id="profile_add_astoria",
    ...
    PromptCase(
        stage="profile", id="profile_has_dog",
        prompt="I have a dog — update my profile",
        expected_agents={"profile_agent"},
        expected_tools={"update_my_profile"},
    ),
    PromptCase(
        stage="profile", id="profile_movein_date",
    ...
    PromptCase(
        stage="profile", id="profile_student",
    ...
    PromptCase(
        stage="profile", id="profile_change_name",
    ...
    PromptCase(
        stage="profile", id="profile_dealbreakers",
        prompt="what are my dealbreakers right now?",
        expected_agents={"profile_agent"},
        expected_tools={"view_my_profile"},
    ),
    PromptCase(
        stage="profile", id="profile_remove_elevator",
...
ROOMMATES = [
    PromptCase(
        stage="roommates", id="rm_find_match",
    ...
    PromptCase(
        stage="roommates", id="rm_night_owl_clean",
    ...
    PromptCase(
        stage="roommates", id="rm_message_first",
    ...
    PromptCase(
        stage="roommates", id="rm_gender_pref",
    ...
    PromptCase(
        stage="roommates", id="rm_withdraw_request",
...
GROUPS = [
    PromptCase(
        stage="groups", id="grp_create_nyc_hunt",
    ...
    PromptCase(
        stage="groups", id="grp_list_mine",
        prompt="show me my groups",
        expected_agents={"groups_agent"},
        expected_tools={"list_my_groups"},
    ),
    PromptCase(
        stage="groups", id="grp_members_of",
        prompt="who's in my NYC Hunt group?",
        expected_agents={"groups_agent"},
        expected_tools={"list_my_groups"},
    ),
    PromptCase(
        stage="groups", id="grp_save_to_group",
    ...
    PromptCase(
        stage="groups", id="grp_remove_bob",
    ...
    PromptCase(
        stage="groups", id="grp_promote_admin",
...
LEASE = [
    PromptCase(
        stage="lease", id="lease_upload_parse",
...
MOVEIN = [
    PromptCase(
        stage="movein", id="mi_tell_me_about_place",
    ...
    PromptCase(
        stage="movein", id="mi_add_internet",
    ...
    PromptCase(
        stage="movein", id="mi_whats_left",
        prompt="what's left before I move?",
        expected_agents={"movein_agent"},
        expected_tools={"list_movein_tasks", "view_movein_plan"},
    ),
    PromptCase(
        stage="movein", id="mi_schedule_movers",
        prompt="I want to schedule movers",
        notes="No order-creation tool today — agent should list orders or "
              "explain, but stay in movein.",
        expected_agents={"movein_agent"},
        forbidden_agents={"search_agent"},
    ),
    PromptCase(
        stage="movein", id="mi_order_cleaning",
        prompt="order cleaning for the day before move-in",
        expected_agents={"movein_agent"},
        forbidden_agents={"search_agent"},
    ),
    PromptCase(
        stage="movein", id="mi_list_orders",
        prompt="show my move-in orders",
        expected_agents={"movein_agent"},
        expected_tools={"list_movein_orders"},
    ),
    PromptCase(
        stage="movein", id="mi_take_photos",
...
BUILDINGS = [
    PromptCase(
        stage="buildings", id="bld_reviews_bedford",
    ...
    PromptCase(
        stage="buildings", id="bld_hpd_at_address",
    ...
    PromptCase(
        stage="buildings", id="bld_leave_review",
...
ATTACHMENTS = [
    PromptCase(
        stage="attachments", id="att_random_photo",
...
CROSS = [
    PromptCase(
        stage="cross_cutting", id="x_undo_last",
    ...
    PromptCase(
        stage="cross_cutting", id="x_switch_brooklyn_to_queens",
    ...
    PromptCase(
        stage="cross_cutting", id="x_journey_summary",
...
COLD_META = [
    PromptCase(stage="cs_meta", id="cs_hey", prompt="hey",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_hello_there", prompt="hello there",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_are_you_human",
        prompt="are you a human?",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_what_is_wade",
        prompt="what is wade me home?",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_i_dont_know",
        prompt="I don't know what I'm doing",
        notes="Vague — can ask clarifying Q or offer onboarding. Must not search blindly.",
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_meta", id="cs_help", prompt="help",
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_meta", id="cs_can_you_find",
        ...
        forbidden_agents={"navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_is_this_free",
        prompt="is this free?",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_thanks", prompt="thanks",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_meta", id="cs_ok_cool", prompt="ok cool",
        forbidden_agents={"search_agent", "navigator_agent"}),
]

COLD_SEARCH = [
    PromptCase(stage="cs_discovery", id="cs_1br_bushwick_3k",
        prompt="I want a 1BR in Bushwick under 3k",
        expected_agents={"search_agent"}, expected_tools={"search_listings"},
        expected_cards={"property_results"}),
    PromptCase(stage="cs_discovery", id="cs_3br_harlem",
        prompt="find 3-bedrooms in Harlem",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_pet_east_village",
        prompt="pet-friendly apartments in the east village",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_studio_fidi",
        prompt="I need a studio in FiDi for under 2500",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_luxury_lic_gym",
        prompt="luxury 2BR in LIC with a gym",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_rent_stabilized_chelsea",
        prompt="rent-stabilized in Chelsea",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_brownstone_park_slope",
        prompt="brownstone in Park Slope with a backyard",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_cheapest_1br_manhattan",
        prompt="cheapest 1BR in Manhattan",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_2br_astoria_laundry",
        prompt="2BR in Astoria under 3500, laundry in-unit",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_duplex_brooklyn",
        prompt="duplex with outdoor space in Brooklyn",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_near_nyu",
        prompt="apartments near NYU",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_family_queens",
        prompt="family-friendly rentals near good schools in Queens",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_furnished_midtown",
        prompt="furnished short-term 1BR in Midtown",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_studio_ues_elevator",
        prompt="studio on the Upper East Side with elevator",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_garden_park_slope",
        prompt="garden-level 1BR in Park Slope",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_williamsburg_roof",
        prompt="2BR in Williamsburg with roof deck",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_modern_lic",
        prompt="modern 1BR in LIC under 3500",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_convertible_2br",
        prompt="convertible 2BR with dishwasher under 4000",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_near_g_train",
        prompt="something near the G train",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
    PromptCase(stage="cs_discovery", id="cs_dumbo_affordable",
        prompt="affordable 1BR in DUMBO",
        expected_agents={"search_agent"}, expected_tools={"search_listings"}),
]

COLD_SAVING = [
    PromptCase(stage="cs_saving", id="cs_show_saved",
        prompt="show my saved apartments",
        expected_agents={"saved_agent"}, expected_tools={"list_saved_properties"},
        expected_cards={"favorites_results"}, forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_saving", id="cs_any_favorites",
        prompt="do I have any favorites saved?",
        expected_agents={"saved_agent"}, expected_tools={"list_saved_properties"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_saving", id="cs_how_many_favorited",
        prompt="how many places have I favorited?",
        expected_agents={"saved_agent"}, expected_tools={"list_saved_properties"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_saving", id="cs_list_saved_properties",
        prompt="list my saved properties",
        expected_agents={"saved_agent"}, expected_tools={"list_saved_properties"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_saving", id="cs_see_favorites",
        prompt="I'd like to see my favorites",
        expected_agents={"saved_agent"}, expected_tools={"list_saved_properties"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_saving", id="cs_what_starred",
...
COLD_TOURS = [
    PromptCase(stage="cs_touring", id="cs_next_tour",
        prompt="when is my next tour?",
        expected_agents={"tours_agent"}, expected_tools={"list_my_tours"},
        expected_cards={"tours_results"}),
    PromptCase(stage="cs_touring", id="cs_any_tours_scheduled",
        prompt="do I have any tours scheduled?",
        expected_agents={"tours_agent"}, expected_tools={"list_my_tours"}),
    PromptCase(stage="cs_touring", id="cs_tours_this_week",
        prompt="what tours do I have this week?",
        expected_agents={"tours_agent"}, expected_tools={"list_my_tours"}),
    PromptCase(stage="cs_touring", id="cs_tour_today",
        prompt="I have a tour today, what time?",
        expected_agents={"tours_agent"}, expected_tools={"list_my_tours"}),
    PromptCase(stage="cs_touring", id="cs_tour_schedule",
        prompt="show my tour schedule",
        expected_agents={"tours_agent"}, expected_tools={"list_my_tours"}),
    PromptCase(stage="cs_touring", id="cs_book_123_main_sat",
        prompt="book a tour at 123 Main St on Saturday",
        expected_agents={"tours_agent"}, expected_tools={"schedule_tour"}),
    PromptCase(stage="cs_touring", id="cs_schedule_100_park_2pm",
        prompt="schedule a showing at 100 Park Ave tomorrow at 2pm",
        expected_agents={"tours_agent"}, expected_tools={"schedule_tour"}),
    PromptCase(stage="cs_touring", id="cs_upcoming_tours",
        prompt="upcoming tours",
        expected_agents={"tours_agent"}, expected_tools={"list_my_tours"}),
]

COLD_PROFILE = [
    PromptCase(stage="cs_profile", id="cs_show_preferences",
        prompt="show my preferences",
        expected_agents={"profile_agent"}, expected_tools={"view_my_profile"},
        expected_cards={"profile_summary"}, forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_profile", id="cs_search_criteria",
        prompt="what are my search criteria?",
        expected_agents={"profile_agent"}, expected_tools={"view_my_profile"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_profile", id="cs_update_budget_4000",
        prompt="update my budget to 4000",
        expected_agents={"profile_agent"}, expected_tools={"update_my_profile"}),
    PromptCase(stage="cs_profile", id="cs_prefer_elevator",
        prompt="I prefer elevator buildings",
        expected_agents={"profile_agent"}, expected_tools={"update_my_profile"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_profile", id="cs_allergic_cats",
        prompt="I'm allergic to cats",
        expected_agents={"profile_agent"}, expected_tools={"update_my_profile"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_profile", id="cs_add_doorman",
        prompt="add doorman to must-haves",
        expected_agents={"profile_agent"}, expected_tools={"update_my_profile"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_profile", id="cs_want_outdoor",
        prompt="I want outdoor space",
        notes="Ambiguous — profile pref or search filter. Either is fine; "
              "just don't route to navigator.",
        forbidden_agents={"navigator_agent"}),
    PromptCase(stage="cs_profile", id="cs_movein_july_15",
        prompt="my target move-in is July 15",
        notes="Either profile_agent (move-in date pref) or movein_agent (plan). "
              "Must not search.",
        forbidden_agents={"search_agent", "navigator_agent"}),
    PromptCase(stage="cs_profile", id="cs_no_pets",
        prompt="I don't have any pets",
        expected_agents={"profile_agent"}, expected_tools={"update_my_profile"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_profile", id="cs_whats_max_rent",
        prompt="what's my max rent?",
        expected_agents={"profile_agent"}, expected_tools={"view_my_profile"}),
]

COLD_ROOMMATES = [
    PromptCase(stage="cs_roommates", id="cs_find_matches",
        prompt="find me roommate matches",
        expected_agents={"roommates_agent"},
        expected_tools={"list_roommate_matches"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_roommates", id="cs_show_rm_profile",
        prompt="show my roommate profile",
        expected_agents={"roommates_agent"},
        expected_tools={"view_my_roommate_profile"},
        forbidden_agents={"profile_agent"}),
    PromptCase(stage="cs_roommates", id="cs_looking_rm_brooklyn",
        prompt="I'm looking for a roommate in Brooklyn",
        expected_agents={"roommates_agent"},
        expected_tools={"list_roommate_matches"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_roommates", id="cs_top_matches",
        prompt="who are my top roommate matches?",
        expected_agents={"roommates_agent"},
        expected_tools={"list_roommate_matches"}),
    PromptCase(stage="cs_roommates", id="cs_quiet_tidy",
        prompt="I'm quiet, tidy, non-smoker",
        notes="Roommate personality — roommates_agent NOT profile_agent.",
        expected_agents={"roommates_agent"},
        expected_tools={"update_my_roommate_profile"},
        forbidden_agents={"profile_agent"}),
    PromptCase(stage="cs_roommates", id="cs_rm_connections",
        prompt="what roommates am I connected to?",
        expected_agents={"roommates_agent"},
        expected_tools={"list_my_connections"}),
    PromptCase(stage="cs_roommates", id="cs_28_tech",
        prompt="I'm 28, I work in tech, looking for a roommate",
        expected_agents={"roommates_agent"},
        forbidden_agents={"profile_agent", "search_agent"}),
    PromptCase(stage="cs_roommates", id="cs_update_rm_profile",
...
COLD_GROUPS = [
    PromptCase(stage="cs_groups", id="cs_create_group_hunt",
        prompt="create a group for my apartment hunt",
        expected_agents={"groups_agent"}, expected_tools={"create_group"}),
    PromptCase(stage="cs_groups", id="cs_show_groups",
        prompt="show my groups",
        expected_agents={"groups_agent"}, expected_tools={"list_my_groups"}),
    PromptCase(stage="cs_groups", id="cs_start_cotenant",
        prompt="start a co-tenant group",
        expected_agents={"groups_agent"}, expected_tools={"create_group"}),
    PromptCase(stage="cs_groups", id="cs_invite_to_group",
        prompt="invite someone to my group",
        expected_agents={"groups_agent"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_groups", id="cs_list_groups",
        prompt="list my groups",
        expected_agents={"groups_agent"}, expected_tools={"list_my_groups"}),
]

COLD_LEASE = [
    PromptCase(stage="cs_lease", id="cs_review_lease",
        prompt="review my lease",
        expected_agents={"lease_agent"},
        expected_tools={"view_my_lease", "ask_about_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_when_expire",
        prompt="when does my lease expire?",
        expected_agents={"lease_agent"},
        expected_tools={"ask_about_my_lease", "view_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_monthly_rent_lease",
        prompt="what's my monthly rent on my lease?",
        expected_agents={"lease_agent"},
        expected_tools={"ask_about_my_lease", "view_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_guest_rules",
        prompt="what are the rules about guests in my lease?",
        expected_agents={"lease_agent"},
        expected_tools={"ask_about_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_sublet_allowed",
        prompt="does my lease allow subletting?",
        expected_agents={"lease_agent"},
        expected_tools={"ask_about_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_summarize_lease",
        prompt="summarize my lease",
        expected_agents={"lease_agent"},
        expected_tools={"view_my_lease", "ask_about_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_explain_lease",
        prompt="explain my lease to me",
        expected_agents={"lease_agent"},
        expected_tools={"view_my_lease", "ask_about_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_deposit_amount",
        prompt="what's the security deposit on my lease?",
        expected_agents={"lease_agent"},
        expected_tools={"ask_about_my_lease", "view_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_notice_period",
        prompt="what's my notice period?",
        expected_agents={"lease_agent"},
        expected_tools={"ask_about_my_lease"}),
    PromptCase(stage="cs_lease", id="cs_pull_up_lease",
...
COLD_MOVEIN = [
    PromptCase(stage="cs_movein", id="cs_movein_date",
        prompt="when is my move-in date?",
        expected_agents={"movein_agent"},
        expected_tools={"view_movein_plan"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_movein", id="cs_movein_checklist",
        prompt="show my move-in checklist",
        expected_agents={"movein_agent"},
        expected_tools={"list_movein_tasks", "view_movein_plan"}),
    PromptCase(stage="cs_movein", id="cs_before_move",
        prompt="what do I need to do before I move?",
        expected_agents={"movein_agent"},
        expected_tools={"list_movein_tasks", "view_movein_plan"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_movein", id="cs_where_moving",
        prompt="where am I moving to?",
        expected_agents={"movein_agent"},
        expected_tools={"view_movein_plan"},
        forbidden_agents={"search_agent"},
        forbidden_tools={"search_listings"}),
    PromptCase(stage="cs_movein", id="cs_movein_address",
        prompt="what's my move-in address?",
        expected_agents={"movein_agent"},
        expected_tools={"view_movein_plan"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_movein", id="cs_movein_orders",
        prompt="show my move-in orders",
        expected_agents={"movein_agent"},
        expected_tools={"list_movein_orders"}),
    PromptCase(stage="cs_movein", id="cs_set_up_utilities",
        prompt="I need to set up utilities",
        expected_agents={"movein_agent"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_movein", id="cs_help_with_move",
        prompt="help me with my move",
        expected_agents={"movein_agent"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_movein", id="cs_list_movein_tasks",
        prompt="list my move-in tasks",
        expected_agents={"movein_agent"},
        expected_tools={"list_movein_tasks", "view_movein_plan"}),
    PromptCase(stage="cs_movein", id="cs_left_before_day",
...
COLD_GUARANTOR = [
    PromptCase(stage="cs_guarantor", id="cs_do_I_need",
        prompt="do I need a guarantor?",
        expected_agents={"guarantor_agent"},
        expected_tools={"view_guarantor_center"}),
    PromptCase(stage="cs_guarantor", id="cs_show_guarantors",
        prompt="show my guarantors",
        expected_agents={"guarantor_agent"},
        expected_tools={"view_guarantor_center"}),
    PromptCase(stage="cs_guarantor", id="cs_saved_guarantors",
...
COLD_BUILDINGS = [
    PromptCase(stage="cs_buildings", id="cs_tell_100_bedford",
        prompt="tell me about 100 Bedford Ave",
        expected_agents={"buildings_agent"},
        expected_tools={"lookup_building"},
        forbidden_agents={"search_agent"}),
    PromptCase(stage="cs_buildings", id="cs_hpd_250_grand",
        prompt="HPD violations at 250 Grand St",
        expected_agents={"buildings_agent"},
        expected_tools={"list_hpd_violations_for_building", "lookup_building"}),
    PromptCase(stage="cs_buildings", id="cs_reviews_350_lex",
        prompt="reviews of 350 Lexington Ave",
        expected_agents={"buildings_agent"},
        expected_tools={"list_reviews_for_building", "lookup_building"}),
    PromptCase(stage="cs_buildings", id="cs_lookup_500_broadway",
        prompt="look up 500 Broadway, NYC",
        expected_agents={"buildings_agent"},
        expected_tools={"lookup_building"}),
    PromptCase(stage="cs_buildings", id="cs_dob_wtc",
        prompt="DOB complaints at 1 World Trade Center",
        expected_agents={"buildings_agent"},
        expected_tools={"list_dob_complaints_for_building", "lookup_building"}),
    PromptCase(stage="cs_buildings", id="cs_is_320w37_good",
        prompt="is 320 West 37th a good building?",
        expected_agents={"buildings_agent"},
        expected_tools={"lookup_building"}),
    PromptCase(stage="cs_buildings", id="cs_owns_432_park",
        prompt="who owns 432 Park Avenue?",
        expected_agents={"buildings_agent"},
        expected_tools={"lookup_building"}),
    PromptCase(stage="cs_buildings", id="cs_durst_reviews",
